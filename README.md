...
│
└── web/                    # 🌐 public website + the same terminal as a web app (PWA)
    ├── index.html          #   landing page (EN/සිංහල, live prices, FAQ)
    ├── app/                #   the web app (synced from the Android assets on every deploy)
    ├── manifest.webmanifest · sw.js   # PWA: installable, offline shell
    ├── icons/ · og-cover.png          # generated by tools/make_icons.py
...
Deploy: repo → Settings → Pages → Source: **GitHub Actions** → `main` එකට merge කරන්න
(හෝ **Deploy website** workflow එක අතින් run කරන්න). විස්තර: [`web/README.md`](web/README.md).
